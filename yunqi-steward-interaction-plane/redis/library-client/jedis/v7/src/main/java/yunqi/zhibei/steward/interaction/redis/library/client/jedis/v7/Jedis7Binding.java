...
/** Creates, checks, and closes a native Jedis 7 pooled client. */
public final class Jedis7Binding
        implements ResourceBinding<Jedis7Configuration, JedisPooled> {

    public static BoundResource<JedisPooled> start(Jedis7Configuration configuration)
            throws Exception {
        return BoundResource.start(configuration, new Jedis7Binding());
    }

    private static final String EXPECTED_JEDIS_MAJOR_VERSION = "7";
    private static final String VERSION_RESOURCE =
            "META-INF/maven/redis.clients/jedis/pom.properties";

    public Jedis7Binding() {
        verifyDependencyMajorVersion();
    }

    @Override
    public JedisPooled create(Jedis7Configuration configuration) {
    ...
    static DefaultJedisClientConfig clientConfig(Jedis7Configuration configuration) {
        Objects.requireNonNull(configuration, "configuration");
    ...
    static GenericObjectPoolConfig<Connection> poolConfig(Jedis7Configuration configuration) {
        Objects.requireNonNull(configuration, "configuration");
    ...
    static void verifyDependencyMajorVersion() {
        ...
        if (!EXPECTED_JEDIS_MAJOR_VERSION.equals(actualMajorVersion)) {
            throw new IllegalStateException(
                    "Jedis 7 binding requires dependency major version "
